...
{-
    ...
    Blind calls will need to adhere to ABI
...
unclobber is =
    ...
    let state = Liveness { writeSet   = mempty
                         , readSet    = mempty
                         , liveness   = mempty
                         , clobbered  = mempty
                         , callBlocks = mempty
...
findClobbered = zipWithM_ go [0..]
    ...
    go n (CCall (CallLabel lbl) prePushes postPops) =
        knownCall n lbl prePushes postPops
...
knownCall :: Ord s => Int -> s -> Int -> Int -> Unclobber s ()
knownCall lineNo lbl prePushes postPops = lift . modify' $ knownCall'

    where
    knownCall' live =

        let liveness' = M.insert (lineNo, lbl)
