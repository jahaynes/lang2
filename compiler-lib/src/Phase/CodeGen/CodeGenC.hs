...
codeGenCApp _ f xs = do
    ...
    let envPush = CPush (CLitInt 0) -- Hack.  TODO - what is this?
    ...
    let call = case f' of
                   ...
                   CReg r ->
                       -- Assume closure?
                       -- Blind call.  Cannot statically know the callee due to branching
                       -- (Therefore must preserve regs in accordance with ABI)
                       CallReg r
    ret <- freshReg
